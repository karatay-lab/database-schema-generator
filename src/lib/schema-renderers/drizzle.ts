...
function drizzleDefault(defaultVal: string, provider: string, colExpr: string): string {
  ...
  if (defaultVal === "now()") return provider === "sqlite" ? ".$defaultFn(() => new Date())" : ".defaultNow()";
  ...
  if (/^["'].*["']$/.test(defaultVal)) {
    try { return `.default(${JSON.stringify(JSON.parse(defaultVal))})`; } catch { return `.default(${defaultVal})`; }
  }
...
function pgCol(field: FieldShape, colName: string, isPk: boolean, used: Set<string>): string {
  const { type } = field;
  const nat = nativeOf(field.constraints);
  const isAutoUuid = field.default.includes("gen_random_uuid") || field.default === "uuid()";
  if (type === "string" || !SCALAR_TYPES.has(type)) {
    if (nat?.name === "Uuid") { used.add("uuid"); return `uuid("${colName}")${isPk ? ".primaryKey()" : ""}${isAutoUuid ? ".defaultRandom()" : ""}`; }
    if (nat?.name === "VarChar") { const len = nat.args?.[0] ? parseInt(nat.args[0], 10) : 255; used.add("varchar"); return `varchar("${colName}", { length: ${isNaN(len) ? 255 : len} })${isPk ? ".primaryKey()" : ""}`; }
    used.add("text"); return `text("${colName}")${isPk ? ".primaryKey()" : ""}`;
  }
  if (type === "integer") {
    ...
    used.add("integer"); return `integer("${colName}")${isPk ? ".primaryKey()" : ""}`;
  }
  ...
  if (type === "timestamp") { used.add("timestamp"); return nat?.name === "Timestamptz" ? `timestamp("${colName}", { withTimezone: true })` : `timestamp("${colName}")`; }
...
function sqliteCol(field: FieldShape, colName: string, isPk: boolean, used: Set<string>): string {
  const { type } = field;
  if (type === "string" || !SCALAR_TYPES.has(type)) { used.add("text"); return `text("${colName}")${isPk ? ".primaryKey()" : ""}`; }
  if (type === "integer") { used.add("integer"); if (isPk && field.default === "autoincrement()") return `integer("${colName}").primaryKey({ autoIncrement: true })`; return `integer("${colName}")${isPk ? ".primaryKey()" : ""}`; }
  ...
  used.add("text"); return `text("${colName}") /* enum: ${type} */`;
}

function mysqlCol(field: FieldShape, colName: string, isPk: boolean, used: Set<string>): string {
  const { type } = field;
  const nat = nativeOf(field.constraints);
  if (type === "string" || !SCALAR_TYPES.has(type)) {
    if (nat?.name === "Uuid") { used.add("varchar"); return `varchar("${colName}", { length: 36 })${isPk ? ".primaryKey()" : ""}`; }
    if (nat?.name === "VarChar") { const len = nat.args?.[0] ? parseInt(nat.args[0], 10) : 255; used.add("varchar"); return `varchar("${colName}", { length: ${isNaN(len) ? 255 : len} })${isPk ? ".primaryKey()" : ""}`; }
    used.add("text"); return `text("${colName}")${isPk ? ".primaryKey()" : ""}`;
  }
  if (type === "integer") { if (isPk && field.default === "autoincrement()") { used.add("serial"); return `serial("${colName}")`; } used.add("int"); return `int("${colName}")${isPk ? ".primaryKey()" : ""}`; }
  if (type === "bigint") { used.add("bigint"); return `bigint("${colName}", { mode: "number" })${isPk ? ".primaryKey()" : ""}`; }
...
export function generateDrizzleSchema(
  store: ReturnType<typeof graphToCanonicalStore>,
): string {
    ...
    for (const field of model.fields) {
      ...
      if (field.relation && relationFields.length > 0) {
        relationFields.forEach((localName, i) => {
          fkMap.set(localName, { targetModel: field.type, targetRef: field.relation?.references?.[i] ?? "id" });
        });
      }
    }
