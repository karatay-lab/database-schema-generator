...
function nativeOf(
  ...
  return constraints.find(
    (c): c is NativeConstraint => c.type === "NATIVE",
  );
}
...
function drizzleDefault(defaultVal: string, provider: string, colExpr: string): string {
  ...
  // Complex / unknown – add as comment
  return ` /* .default(${defaultVal}) */`;
}
...
function pgCol(
  ...
  // Enum: use the generated enum fn variable
...
function generateDrizzleSchema(
  ...
  // Enum definitions (PG / MySQL only)
  ...
  for (const model of store.models) {
    ...
    // Build FK map: localScalarFieldName → { targetModel, targetRef }
    ...
    for (const field of model.fields) {
      // Skip relation fields (both FK-owner and back-reference)
    ...
    const hasRestrictions = restrictionLines.length > 0;

    const block = hasRestrictions
...
export async function GET(request: Request) {
  ...
  if (type !== "prisma" && type !== "drizzle") {
    return NextResponse.json(
      { error: "Export type must be 'prisma' or 'drizzle'." },
      { status: 400 },
    );
  }
